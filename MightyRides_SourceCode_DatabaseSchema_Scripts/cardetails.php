            ...
            FROM CAR 
                ...
                images = (  select  images 
                            ...
                            where 
                                carid = car.carid
                            limit 1 )
            order by uploadedtime desc"; //to select cars along with its status

if($result = $conn->query($query))
{   
    $arr = array();

    // Fetch an individual row frow query
    while($row = $result->fetch_array(MYSQLI_ASSOC))
    {
        /* Two possibilities:-
            1. Either a car is new
            2. Car is preowned(for resale)
        */
