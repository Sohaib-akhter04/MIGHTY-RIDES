...
$query1 = "select car.carid, car.dealerid from car inner join owns where owns.carid=car.carid and owns.dealerid = $dealerid"; //order by uploadedtime desc";
$result1 = mysqli_query($conn, $query1);

//second query to get whether any cars have been sold out
$statusquery = "select count(status) as soldoutcount from car inner join owns where owns.carid=car.carid and owns.dealerid = $dealerid and status='sold out'";
...
car.carid=owns.carid and enddate is null and owns.dealerid = $dealerid and status='rented'";  // ************ EDITED
...
<head>
  <title><?php echo $dealername . "'s " ?> Dashboard - MigtyRides</title>
...
-->
    ...
    <div class="container" style="display:flex;flex-direction:column;align-items:center">
      ...
      while ($row = mysqli_fetch_assoc($result1)) {
        ...
        $mainquery = "select  car.carid as carid,
                        name,
                        status,
                        cartype,
                        images 
                from car 
                  left join images 
                        on images.carid = car.carid 
                    and images = (  select images from images 
                                    where carid = car.carid limit 1)
                having car.carid = " . $row["carid"];
