...
$dealer_insert= "INSERT INTO DEALER (DName, PhoneNo, Website, D_Email, D_password) VALUES (?,?,?,?,?)";
...
if($stmt= mysqli_prepare($conn, $dealer_insert) )
{
    //Bind the variables to prepared statements as parameters
     mysqli_stmt_bind_param($stmt, "sssss", $d_name, $d_phoneno, $d_website, $d_email, $d_password);
...
if($stmt = mysqli_prepare($conn, $dealer_login))
{
    ...
    if(mysqli_stmt_execute($stmt))
    {
        ...
        $message = "Hello $d_name, you are one step away from becoming a MightyRider! <a href= 'https://localhost/CARS/verify.php?vkey=$vkey'>Click Here</a> to verify your email address and activate your account!";
