...
<html>
<head>
    <title><?php echo $dealername."'s " ?> Sales - MightyRides</title>
...
$soldcarsquery = "select    customername,
                            car.carid as carid,
                            name,DATE_FORMAT(paymentdate,'%d %M %Y') as paymentdate 
                from owns 
                    inner join customer
                    inner join car 
                    inner join newcar 
                where       car.carid=newcar.newcarid 
                    and     owns.dealerid = $dealerid 
                    and     paymentstatus='verified'
                    and     customer.customerid = newcar.customerid
                    and     car.carid = owns.carid 
...
// FOR PRE-OWNED CARS SOLD

$soldresalecarsquery = "select      customername,
                        ...
                        from owns 
                            inner join customer
                            inner join car
                            inner join preownedcar
                        where       
                                    car.carid = preownedcar.preownedcarid 
                            and     owns.dealerid = $dealerid 
                            and     paymentstatus = 'verified'
                            and     customer.customerid = preownedcar.customerid 
                            and     car.carid = owns.carid 
                        order by paymentdate desc";
...
<div class="card-body">
<h5 class="card-title"><?php echo $row["name"] ?></h5>
<h6 class="card-subtitle mb-2">Sold on <?php echo $row["paymentdate"]." to ".$row["customername"] . " (User id: " .$row["customerid"] .")" ?></h6>
<hr>
