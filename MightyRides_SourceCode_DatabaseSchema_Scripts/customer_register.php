...
$customer_insert= "INSERT INTO CUSTOMER (CustomerName, DOB, PhoneNo, Address, DrivingLicense, C_Email, password) VALUES (?,?,?,?,?,?,?)";
//set parameters
...
if($stmt= mysqli_prepare($conn, $customer_insert) )
{
    //Bind the variables to prepared statements as parameters
     mysqli_stmt_bind_param($stmt, "sssssss", $c_name,  $c_dob, $c_phoneno, $c_address, $c_drivingLicense, $c_email, $c_password);
...
if($stmt = mysqli_prepare($conn, $customer_login))
{
    ...
    if(mysqli_stmt_execute($stmt))
    {
        ...
        $subject = "MigtyRides Email Verification";
        $message = "Hello $c_name, you are one step away from becoming a MigtyRider! <a href= 'https://localhost/CARS/verify.php?vkey=$vkey'>Click Here</a> to verify your email address and activate your account!";
        $headers = "From: thisisourprojectx@gmail.com" . "\r\n";
        // Always set content-type when sending HTML email
        $headers .= "MIME-Version: 1.0" . "\r\n";
        $headers .= "Content-type:text/html;charset=UTF-8" . "\r\n";
        // echo "SIUU";
        // echo mail($c_email,$subject,$message); //,$headers);
